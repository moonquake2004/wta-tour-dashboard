...
| `calendar.html` | 赛程 | 全部主巡回赛赛事，按月份分组，含级别、场地、签位、奖金、冠军；可按状态筛选（全部 / 进行中 / 已完成 / 未开始） |
...
**不用 JavaScript 的交互**：语言切换、赛程状态筛选、预渲染排序、弹窗都用 CSS `:target` 实现——
每个筛选条件都是一份预渲染好的完整列表，切换状态不重新渲染任何内容。
三个锚点位于 `<body>` 顶部、与正文同级，因此
`#lang-cn:target ~ .lang-ctx .en { display: none }` 仅靠 URL 片段就能切换语言。
球员头像的兜底也是纯 CSS 图层，缺图不会出现破图图标。

---
唯一的例外：交手页的两个原生下拉挂了一个约 15 行的小脚本，选完 A+B 会直接
跳到对阵页；脚本禁用时自动退化为纯链接的双列选择，整站依然可用。
