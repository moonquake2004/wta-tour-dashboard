...
| 赛程 | Calendar | 全部主巡回赛赛事：级别、场地、签位、奖金、冠军；**点击任意赛事查看该站完整单打赛果** |
...
| 交手 | Head-to-head | 任意两位球员：交手记录、逐场明细、数据对比。选择器覆盖**全部排名球员（前 100 名全部可选）**，空查询时按排名浏览，支持中英文姓名搜索，并自动排除对方已选的球员 |

- **默认中英对照**：所有球员、赛事、国家/地区、轮次、场地都同时显示中英文；
  页头三档切换（`中/EN · 中文 · EN`）可收敛为单一语言，选择本地保存。
  切换完全由 CSS（`html[data-lang]`）驱动，切换语言不触发任何重新渲染。
- **配色取自四大满贯官方品牌色**（实测各赛事官网 CSS）：

  | 赛事 | 官网实测主色 |
  | --- | --- |
  | 温网 | 深绿 `#00552b` / `#00331a`、紫 `#540082`、绿金 `#816c3c` |
  | 澳网 | 藏青 `#1e2886`、深蓝 `#003a5d`、亮蓝 `#0092d3` |
  | 法网 | 陶土橙 `#cc4e0e` / `#e38045`、深绿 `#00503c` / `#033629` |
  | 美网 | 蓝 `#2478cc` / `#00288c`、金 `#ffd400` |

  网站以温网场地绿为底（为长时间阅读做了提亮：大面积底色感知明度 L*≈29，页面均值 L*≈37），
  文字刻意压制在纯白之下（正文 L*≈92、强调数字 L*≈95），避免深绿底上的眩光，**金色严格只用于荣誉位**——每场胜者、冠军、前三名、
  大满贯级别标签；**🏆 只出现在真正的冠军位置**（冠军墙、赛程冠军、赛事赛果弹窗顶部），
  不会出现在单场胜者旁。三块场地色仅用于标识场地类型。字体中文用 Noto Serif SC、
  拉丁与数字用 Oswald / Barlow Condensed。抽样对比度 6.6:1 ~ 14.9:1 全部满足 WCAG AA。
- **球员详情弹窗**：页面任意位置点击球员，查看分赛季战绩、发球数据、最高排名与奖金。
- **赛事赛果弹窗**：在赛程里点击任意赛事，按轮次查看该站**全部单打赛果**（含资格赛），
  逐场显示种子、国家、比分与胜者，带抢七与退赛标注。数据来自官方赛事接口
